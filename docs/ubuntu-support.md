...
- CLI build/dry-run validation with the same profile, desktop/session, display manager, kernel, firmware, localization, users, network, Wi-Fi, Bluetooth, audio, browser, extra package, bootloader, and auto-resize knobs used by Alpine.
- TUI distribution selector and Ubuntu package search path.
...
- The GUI is still largely Alpine-worded; CLI/TUI are the supported Ubuntu discovery/build paths in this branch.
- Full Ubuntu image builds require root/privileged loop-device access plus `debootstrap`/GRUB tooling. macOS uses a privileged Ubuntu Docker container path analogous to the Alpine Docker flow.
