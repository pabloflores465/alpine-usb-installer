...
- Gentoo dry-run validation through `ledit_core/backend/scripts/configure-gentoo-usb.sh`:
...
On macOS, `ledit_core/backend/scripts/build-gentoo-usb.sh` automatically re-enters itself in a privileged `linux/amd64` Docker container, matching the shared Linux builder pattern. The first run builds a cached `gentoo builder image` image from `ledit_core/backend/docker/Dockerfile.gentoo-builder`; later runs reuse it.
