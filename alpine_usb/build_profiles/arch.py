...
def arch_packages_from_env(env: dict[str, str]) -> list[str]:
    desktop = env.get("ALPINE_USB_DESKTOP", "xfce").lower()
    wms = [part for part in re.split(r"[\s,]+", env.get("ALPINE_USB_TILING_WMS", "").strip()) if part]
    display_manager = env.get("ALPINE_USB_DISPLAY_MANAGER", "auto").lower()
    network = env.get("ALPINE_USB_NETWORK", "networkmanager").lower()
    audio = env.get("ALPINE_USB_AUDIO", "pipewire").lower()
    browser = env.get("ALPINE_USB_BROWSER", "firefox").lower()
    firmware = env.get("ALPINE_USB_FIRMWARE", "full").lower()
    bootloader = env.get("ALPINE_USB_BOOTLOADER", "grub").lower()
    kernel = env.get("ALPINE_USB_KERNEL_FLAVOR", "lts").lower()
    extra = [part for part in re.split(r"\s+", env.get("ALPINE_USB_EXTRA_PACKAGES", "").strip()) if part]
    ...
    if _enabled(env.get("ALPINE_USB_AUTO_RESIZE", "1")):
    ...
    if graphical:
        ...
        if _enabled(env.get("ALPINE_USB_LEGACY_X11_DRIVERS", "1")):
            _append(
                packages,
                "xf86-video-amdgpu",
                "xf86-video-ati",
