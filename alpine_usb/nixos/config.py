...
from alpine_usb.nixos.packages import NIXOS_DEFAULT_CHANNEL, validate_nix_channel, validate_nix_package_name
...
def config_from_args(args: argparse.Namespace, extra_packages: str) -> NixosBuildConfig:
    ...
    return NixosBuildConfig(
        channel=channel,
        arch=arch,
        hostname=getattr(args, "hostname", "nixos-usb"),
...
def generate_flake_nix(config: NixosBuildConfig) -> str:
    return f"""{{
  description = "Linux USB Utility generated NixOS image";
