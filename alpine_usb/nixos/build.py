...
from alpine_usb.nixos.config import NixosBuildConfig, generate_configuration_nix, generate_flake_nix
...
def env_to_config(env: dict[str, str]) -> NixosBuildConfig:
    ...
    channel = env.get("NIXOS_CHANNEL") or env.get("ALPINE_BRANCH") or "nixos-24.11"
    ...
    return NixosBuildConfig(
        channel=channel,
        arch=arch,
        hostname=env.get("NIXOS_USB_HOSTNAME", "nixos-usb"),
