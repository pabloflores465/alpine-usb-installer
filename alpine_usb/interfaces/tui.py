...
CHOICES = {
    ...
    "release": ["24.04", "noble", "22.04", "jammy"],
    "arch": ["x86_64"],
...
class TuiApp:
    ...
    def extra_packages_screen(self):
        while True:
            items = [
                ...
                ("Search official packages", "top 10 suggestions from distro repositories"),
    ...
    def package_search_screen(self):
        ...
        try:
            if self.config["distro"] == "ubuntu":
                results = cli.search_official_apt_packages(self.config["release"], self.config["arch"], query, 10)
            else:
            ...
            if choice == 0:
                ...
                self.message(
                    ...
                    "\n".join(
                        ...
                        if (k.startswith(("ALPINE_USB_", "UBUNTU_USB_")) and k not in hidden)
                        or k in {"IMAGE_SIZE", "ALPINE_BRANCH", "UBUNTU_RELEASE", "ARCH", "DISTRO"}
                    ),
                )
