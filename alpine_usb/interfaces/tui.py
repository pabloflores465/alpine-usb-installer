...
CHOICES = {
    ...
    "release": ["stable", "testing", "sid", "bookworm", "trixie"],
    "arch": ["x86_64", "amd64"],
...
class TuiApp:
    ...
    def package_search_screen(self):
        query = self.prompt(f"Search {self.config['distro'].title()} packages", "")
        if not query:
            return
        self.status = f"Searching {self.config['distro']} package indexes for '{query}'…"
        self.draw_wait("Package search", self.status)
        try:
            if self.config["distro"] == "debian":
                results = cli.search_official_deb_packages(self.config["release"], self.config["arch"], query, 10)
            else:
            ...
            if choice == 0:
                ...
                self.message(
                    ...
                    "\n".join(
                        ...
                        if (k.startswith(("ALPINE_USB_", "DEBIAN_USB_")) and k not in hidden)
                        or k in {"IMAGE_SIZE", "ALPINE_BRANCH", "DEBIAN_RELEASE", "ARCH", "LINUX_USB_DISTRO"}
                    ),
                )
