...
class Main(QWidget):
    ...
    def config_summary_html(self, output_path: str, env: dict[str, str]) -> str:
        ...
        if distro == "nixos":
            distro_label = "NixOS"
            branch_display = env.get("NIXOS_CHANNEL", env.get("ALPINE_BRANCH", ""))
        elif prefix == "OPENSUSE_USB":
            distro_label = "openSUSE"
            branch_display = env.get("OPENSUSE_RELEASE", env.get("ALPINE_BRANCH", ""))
        else:
            distro_label = "Alpine"
            branch_display = env.get("ALPINE_BRANCH", "")
        rows = [
            ("Output", html_soft_break(output_path)),
            (
                "Image",
                f"{esc(env['IMAGE_SIZE'])} · {distro_label} {esc(branch_display)} · Arch {esc(env['ARCH'])}",
            ),
            (
                "System",
