...
def slackware_cache_dir() -> Path:
    explicit = os.environ.get("ALPINE_USB_SLACKWARE_CACHE_DIR") or os.environ.get("SLACKWARE_USB_CACHE_DIR")
    if explicit:
        return Path(explicit).expanduser()
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".cache"
    return base / "alpine-usb-installer" / "slackware-packages"
...
def _cache_enabled() -> bool:
    return os.environ.get("ALPINE_USB_PACKAGE_CACHE", "1").lower() not in {"0", "no", "false", "off"}


def _cache_ttl_seconds() -> int:
    raw = os.environ.get("ALPINE_USB_PACKAGE_CACHE_TTL", str(DEFAULT_CACHE_TTL_SECONDS))
    try:
        return int(raw)
    except ValueError:
