...
USER_NAME="${DEBIAN_USB_USER:-${ALPINE_USB_USER:-debian}}"
USER_PASSWORD="$(read_secret_value DEBIAN_USB_PASSWORD_FILE DEBIAN_USB_PASSWORD "$(read_secret_value ALPINE_USB_PASSWORD_FILE ALPINE_USB_PASSWORD debian)")"
ROOT_PASSWORD="$(read_secret_value DEBIAN_USB_ROOT_PASSWORD_FILE DEBIAN_USB_ROOT_PASSWORD "$(read_secret_value ALPINE_USB_ROOT_PASSWORD_FILE ALPINE_USB_ROOT_PASSWORD "$USER_PASSWORD")")"
HOSTNAME="${DEBIAN_USB_HOSTNAME:-${ALPINE_USB_HOSTNAME:-debian-usb}}"
TIMEZONE="${DEBIAN_USB_TIMEZONE:-${ALPINE_USB_TIMEZONE:-UTC}}"
LOCALE="${DEBIAN_USB_LOCALE:-${ALPINE_USB_LOCALE:-en_US.UTF-8}}"
LANGUAGE_VALUE="${DEBIAN_USB_LANGUAGE:-${ALPINE_USB_LANGUAGE:-${LOCALE%%.*}:en}}"
CONSOLE_KEYMAP="${DEBIAN_USB_CONSOLE_KEYMAP:-${ALPINE_USB_CONSOLE_KEYMAP:-us}}"
XKB_LAYOUT="${DEBIAN_USB_XKB_LAYOUT:-${ALPINE_USB_XKB_LAYOUT:-us}}"
XKB_VARIANT="${DEBIAN_USB_XKB_VARIANT:-${ALPINE_USB_XKB_VARIANT:-}}"
XKB_MODEL="${DEBIAN_USB_XKB_MODEL:-${ALPINE_USB_XKB_MODEL:-pc105}}"
DESKTOP="$(lower "${DEBIAN_USB_DESKTOP:-${ALPINE_USB_DESKTOP:-xfce}}")"
TILING_WMS="$(normalize_words "${DEBIAN_USB_TILING_WMS:-${ALPINE_USB_TILING_WMS:-}}")"
DEFAULT_SESSION="$(lower "${DEBIAN_USB_DEFAULT_SESSION:-${ALPINE_USB_DEFAULT_SESSION:-auto}}")"
DISPLAY_MANAGER="$(lower "${DEBIAN_USB_DISPLAY_MANAGER:-${ALPINE_USB_DISPLAY_MANAGER:-auto}}")"
NETWORK_BACKEND="$(lower "${DEBIAN_USB_NETWORK:-${ALPINE_USB_NETWORK:-networkmanager}}")"
WIFI="${DEBIAN_USB_WIFI:-${ALPINE_USB_WIFI:-1}}"
BLUETOOTH="${DEBIAN_USB_BLUETOOTH:-${ALPINE_USB_BLUETOOTH:-1}}"
AUDIO="$(lower "${DEBIAN_USB_AUDIO:-${ALPINE_USB_AUDIO:-pipewire}}")"
BROWSER="$(lower "${DEBIAN_USB_BROWSER:-${ALPINE_USB_BROWSER:-firefox}}")"
FIRMWARE="$(lower "${DEBIAN_USB_FIRMWARE:-${ALPINE_USB_FIRMWARE:-full}}")"
LEGACY_X11_DRIVERS="${DEBIAN_USB_LEGACY_X11_DRIVERS:-${ALPINE_USB_LEGACY_X11_DRIVERS:-1}}"
PROFILE="${DEBIAN_USB_PROFILE:-${ALPINE_USB_PROFILE:-compatibility}}"
BOOTLOADER="$(lower "${DEBIAN_USB_BOOTLOADER:-${ALPINE_USB_BOOTLOADER:-grub}}")"
KERNEL_FLAVOR="$(lower "${DEBIAN_USB_KERNEL_FLAVOR:-${ALPINE_USB_KERNEL_FLAVOR:-stable}}")"
BOOT_TIMEOUT="${DEBIAN_USB_BOOT_TIMEOUT:-${ALPINE_USB_BOOT_TIMEOUT:-3}}"
AUTO_RESIZE="${DEBIAN_USB_AUTO_RESIZE:-${ALPINE_USB_AUTO_RESIZE:-1}}"
EXTRA_PACKAGES="${DEBIAN_USB_EXTRA_PACKAGES:-${ALPINE_USB_EXTRA_PACKAGES:-}}"
DRY_RUN="${DEBIAN_USB_DRY_RUN:-${ALPINE_USB_DRY_RUN:-0}}"
