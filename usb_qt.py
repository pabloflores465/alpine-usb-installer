...
class DeviceDialog(QDialog):
    def __init__(self, parent=None):
        ...
        title = QLabel("Select the USB drive to erase")
        title.setStyleSheet("font-size: 20px; font-weight: bold;")
        layout.addWidget(title)
        help_text = QLabel("Only external/removable drives are shown. Select the USB by device name, size, and volume label.")
        help_text.setWordWrap(True)
        help_text.setStyleSheet("color:#cbd5e1;")
        layout.addWidget(help_text)
        warn = QLabel("⚠ The selected drive will be completely erased when you click Flash USB.")
        warn.setWordWrap(True)
        warn.setStyleSheet("color: #fca5a5; font-weight: bold; background:#3f1d1d; border:1px solid #7f1d1d; border-radius:6px; padding:8px;")
        layout.addWidget(warn)
        self.scan_status = QLabel("")
        self.scan_status.setStyleSheet("color:#93c5fd;")
        layout.addWidget(self.scan_status)
        self.list = QListWidget()
        layout.addWidget(self.list, 1)
        row = QHBoxLayout()
        manual_label = QLabel("Manual override:")
        manual_label.setToolTip("Use only if your USB is not listed and you know the exact whole-disk device path.")
        row.addWidget(manual_label)
        self.manual = QLineEdit()
        self.manual.setPlaceholderText("Optional, e.g. /dev/disk7" if platform.system() == "Darwin" else "Optional, e.g. /dev/sdX")
        self.manual.setToolTip("Advanced: type the whole disk device, not a partition. Example: /dev/disk7, not /dev/disk7s1.")
        row.addWidget(self.manual, 1)
        layout.addLayout(row)
        btns = QHBoxLayout()
        self.use = QPushButton("Use this USB")
        self.use.setIcon(make_button_icon("check"))
        self.use.setStyleSheet("background:#16a34a;color:#ffffff;border:0;border-radius:6px;padding:6px 12px;font-weight:bold;")
        self.refresh = QPushButton("Scan again")
    ...
    def populate(self, show_empty_modal: bool = True):
        ...
        self.scan_status.setText(f"Found {count} USB device{'s' if count != 1 else ''}. Select one below." if count else "No USB devices found.")
        self.update_use_button()
        if show_empty_modal and not self.devices:
            modal(self, "info", APP_TITLE, "No USB devices were found. Connect a USB drive and click Scan again.")
...
class Main(QWidget):
    def __init__(self):
        ...
        self.device.setPlaceholderText("No USB selected yet — click Select USB")
        self.status = QLabel("")
        ...
        self.console_toggle.setStyleSheet("text-align:left;font-size:15px;font-weight:bold;color:#93c5fd;margin:0px;padding:6px 10px;background:#1f2937;border:1px solid #374151;border-radius:6px;")
    ...
    def build(self):
        ...
        self.setStyleSheet("""
            ...
            QLineEdit { background:#1f2937; color:#ffffff; border:1px solid #4b5563; border-radius:6px; padding:5px 8px; min-height:28px; }
            QTextEdit { background:#0b1220; color:#ffffff; border:1px solid #374151; border-radius:6px; }
            QPushButton { background:#2563eb; color:#ffffff; border:0; border-radius:6px; padding:6px 10px; min-height:28px; }
        ...
        """)
        title.setStyleSheet("font-size:24px;font-weight:bold;color:#ffffff;margin:0px;padding:0px;")
        subtitle = QLabel("1) Build or choose an image  →  2) Select the USB  →  3) Flash it. Nothing is erased until you click Flash USB and confirm.")
        subtitle.setWordWrap(True)
        subtitle.setStyleSheet("color:#cbd5e1;margin:0px;padding:0px;font-size:13px;")
        header = QVBoxLayout()
        ...
        img_title = QLabel("1. Image to write")
        img_title.setStyleSheet("font-size:16px;font-weight:bold;color:#93c5fd;margin:0px;padding:0px;")
        layout.addWidget(img_title)
        img_help = QLabel("Choose where the Alpine image should be saved. If it already exists, you can skip building and select a USB below.")
        img_help.setWordWrap(True)
        img_help.setStyleSheet("color:#cbd5e1;font-size:12px;")
        layout.addWidget(img_help)
        ...
        self.build_button.setFixedWidth(160)
        self.build_button.setStyleSheet("background:#16a34a;color:#ffffff;border:0;border-radius:6px;padding:6px 10px;font-weight:bold;min-height:28px;")
        ...
        usb_title.setStyleSheet("font-size:16px;font-weight:bold;color:#93c5fd;margin:0px;padding:0px;")
        layout.addWidget(usb_title)
        usb_help = QLabel("No USB is selected automatically. Click Select USB, review the size/name carefully, then confirm before flashing.")
        usb_help.setWordWrap(True)
        usb_help.setStyleSheet("color:#cbd5e1;font-size:12px;")
        layout.addWidget(usb_help)
        usb_box = QVBoxLayout()
        usb_box.setContentsMargins(0, 0, 0, 0)
        usb_box.setSpacing(0)
        usb_row = QHBoxLayout()
        ...
        self.flash_button.setFixedWidth(160)
        self.flash_button.setEnabled(False)
        self.flash_button.setStyleSheet("""
            QPushButton { background:#dc2626;color:#ffffff;border:0;border-radius:6px;padding:6px 10px;font-weight:bold;min-height:28px; }
        ...
        warn = QLabel("⚠ Flashing permanently erases the selected USB device. Verify the device path before continuing.")
        warn.setWordWrap(True)
        warn.setStyleSheet("color:#fca5a5;font-weight:bold;background:#3f1d1d;border:1px solid #7f1d1d;border-radius:6px;padding:8px;font-size:12px;")
    ...
    def toggle_console(self):
        ...
        self.console_toggle.setText("Show console output ▸" if visible else "Hide console output ▾")
