...
if [ "$(uname -s)" = "Darwin" ] && [ "${UBUNTU_USB_BUILD_IN_DOCKER:-0}" != "1" ]; then
  ...
  exec docker run --rm "${docker_name_args[@]}" --platform linux/amd64 --privileged "${docker_env[@]}" "${docker_mounts[@]}" -w /work ubuntu:24.04 bash -ceu '
    ...
    chmod +x ledit_core/backend/scripts/build-ubuntu-usb.sh ledit_core/backend/scripts/configure-ubuntu-usb.sh
    exec ledit_core/backend/scripts/build-ubuntu-usb.sh
  '
fi
