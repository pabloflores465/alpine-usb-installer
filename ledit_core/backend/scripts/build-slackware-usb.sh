...
if { [ "$(uname -s)" = "Darwin" ] || is_enabled "${SLACKWARE_USB_FORCE_DOCKER:-0}"; } && [ "${SLACKWARE_USB_BUILD_IN_DOCKER:-0}" != "1" ]; then
  ...
  exec docker run --rm "${docker_name_args[@]}" --platform linux/amd64 --privileged \
    ...
    sh -ceu '
      ...
      chmod +x ledit_core/backend/scripts/build-slackware-usb.sh ledit_core/backend/scripts/configure-slackware-usb.sh
      exec ledit_core/backend/scripts/build-slackware-usb.sh
    '
fi
