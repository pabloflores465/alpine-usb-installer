...
BUILDER_DOCKERFILE="${LEDIT_USB_BUILDER_DOCKERFILE:-$PROJECT_ROOT/ledit_core/backend/docker/Dockerfile.builder}"
...
if [ "$(uname -s)" = "Darwin" ] && [ "${LEDIT_USB_BUILD_IN_DOCKER:-0}" != "1" ]; then
  ...
  if [ "${LEDIT_USB_SKIP_BUILDER_CACHE:-0}" != "1" ] && [ -f "$BUILDER_DOCKERFILE" ]; then
    ...
    exec docker run --rm "${docker_name_args[@]}" --platform linux/amd64 --privileged \
      ...
      -w /work \
      "$BUILDER_IMAGE" \
      sh -ceu 'chmod +x ledit_core/backend/scripts/build-alpine-usb.sh ledit_core/backend/scripts/configure-alpine-usb.sh; exec ledit_core/backend/scripts/build-alpine-usb.sh'
  fi
  ...
  exec docker run --rm "${docker_name_args[@]}" --platform linux/amd64 --privileged \
    ...
    sh -ceu '
      ...
      chmod +x ledit_core/backend/scripts/build-alpine-usb.sh ledit_core/backend/scripts/configure-alpine-usb.sh
      exec ledit_core/backend/scripts/build-alpine-usb.sh
    '
fi
...
install_grub_removable_bootloader() {
  ...
  local image="$1"
  local fallback="$PROJECT_ROOT/ledit_core/backend/efi-fallback/BOOTX64.EFI"
  ...
  mkdir -p "$PROJECT_ROOT/ledit_core/backend/efi-fallback"
  cat > "$grub_cfg" <<EOF
set default=0
