...
BUILDER_DOCKERFILE="${GENTOO_USB_BUILDER_DOCKERFILE:-$PROJECT_ROOT/ledit_core/backend/docker/Dockerfile.gentoo-builder}"
...
if { [ "$(uname -s)" = "Darwin" ] || is_enabled "${GENTOO_USB_FORCE_DOCKER:-0}"; } && [ "${GENTOO_USB_BUILD_IN_DOCKER:-0}" != "1" ]; then
  ...
  if [ "${GENTOO_USB_SKIP_BUILDER_CACHE:-0}" != "1" ] && [ -f "$BUILDER_DOCKERFILE" ]; then
    ...
    exec docker run --rm "${docker_name_args[@]}" --platform linux/amd64 --privileged \
      ...
      -w /work \
      "$BUILDER_IMAGE" \
      bash -ceu 'chmod +x ledit_core/backend/scripts/build-gentoo-usb.sh ledit_core/backend/scripts/configure-gentoo-usb.sh; exec ledit_core/backend/scripts/build-gentoo-usb.sh'
  fi
  ...
  exec docker run --rm "${docker_name_args[@]}" --platform linux/amd64 --privileged \
    ...
    sh -ceu '
      ...
      chmod +x ledit_core/backend/scripts/build-gentoo-usb.sh ledit_core/backend/scripts/configure-gentoo-usb.sh
      exec ledit_core/backend/scripts/build-gentoo-usb.sh
    '
fi
...
LEDIT_USB_DRY_RUN=1 "$SCRIPT_DIR/configure-gentoo-usb.sh"
...
USER_PASSWORD="$(read_secret LEDIT_USB_PASSWORD_FILE LEDIT_USB_PASSWORD gentoo)"
...
{
  for name in \
    IMAGE_SIZE ARCH LEDIT_USB_USER LEDIT_USB_HOSTNAME LEDIT_USB_TIMEZONE LEDIT_USB_LOCALE \
    LEDIT_USB_LANGUAGE LEDIT_USB_CONSOLE_KEYMAP LEDIT_USB_XKB_LAYOUT LEDIT_USB_XKB_VARIANT \
    LEDIT_USB_XKB_MODEL LEDIT_USB_DESKTOP LEDIT_USB_TILING_WMS LEDIT_USB_DEFAULT_SESSION \
    LEDIT_USB_DISPLAY_MANAGER LEDIT_USB_NETWORK LEDIT_USB_WIFI LEDIT_USB_BLUETOOTH \
    LEDIT_USB_AUDIO LEDIT_USB_BROWSER LEDIT_USB_FIRMWARE LEDIT_USB_LEGACY_X11_DRIVERS \
    LEDIT_USB_BOOTLOADER LEDIT_USB_KERNEL_FLAVOR LEDIT_USB_BOOT_TIMEOUT LEDIT_USB_AUTO_RESIZE \
    LEDIT_USB_EXTRA_PACKAGES LEDIT_USB_PROFILE GENTOO_STAGE3_BRANCH GENTOO_USE_BINPKGS \
  ...
  printf 'export LEDIT_USB_PASSWORD=%s\n' "$(shell_quote "$USER_PASSWORD")"
  printf 'export LEDIT_USB_ROOT_PASSWORD=%s\n' "$(shell_quote "$ROOT_PASSWORD")"
  printf 'export LEDIT_USB_DRY_RUN=0\n'
} > "$ENV_FILE"
chmod 600 "$ENV_FILE"
...
if ! chroot "$ROOTFS" /bin/bash -lc '. /root/gentoo-build.env; unset LEDIT_USB_PASSWORD_FILE LEDIT_USB_ROOT_PASSWORD_FILE; exec /root/configure-gentoo-usb.sh'; then
  save_portage_cache
  exit 1
fi
save_portage_cache
