...
def test_build_scripts_do_not_leak_spaced_env_into_docker_image() -> None:
    ...
    scripts = sorted((repo / "ledit_core" / "backend" / "scripts").glob("build-*-usb.sh"))
    assert scripts, "expected at least one ledit_core/backend/scripts/build-*-usb.sh script"
    failures: list[str] = []
...
def test_build_arch_usb_passes_extra_packages_via_env_file() -> None:
    ...
    repo = Path(__file__).resolve().parents[2]
    text = (repo / "ledit_core" / "backend" / "scripts" / "build-arch-usb.sh").read_text()
...
def test_prepare_terminal_runtime_copies_nested_builder_dockerfile(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for name in ["build-alpine-usb.sh", "configure-alpine-usb.sh"]:
        (source / "ledit_core" / "backend" / "scripts" / name).write_text(name)
