...
def rhel_cache_dir() -> Path:
    explicit = os.environ.get("LEDIT_USB_RHEL_CACHE_DIR")
    if explicit:
        return Path(explicit).expanduser()
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".cache"
    return base / "ledit" / "rhel-packages"
...
def _read_cache(path: Path) -> list[dict[str, str]] | None:
    ...
    if not isinstance(fetched_at, (int, float)):
        return None
    ttl = int(os.environ.get("LEDIT_USB_RHEL_CACHE_TTL", DEFAULT_CACHE_TTL_SECONDS))
