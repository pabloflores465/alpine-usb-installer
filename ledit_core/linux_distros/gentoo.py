...
def gentoo_cache_dir() -> Path:
    explicit = os.environ.get("LEDIT_USB_GENTOO_CACHE_DIR")
    if explicit:
        return Path(explicit).expanduser()
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".cache"
    return base / "ledit" / "gentoo-packages"
...
def fetch_gentoo_packages() -> list[dict[str, str]]:
    ...
    ttl_raw = os.environ.get("LEDIT_USB_GENTOO_CACHE_TTL", str(DEFAULT_CACHE_TTL_SECONDS))
    try:
        ttl = int(ttl_raw)
    except ValueError:
