...
from ledit_core.image_builds import env_from_build_args
from ledit_core.interfaces import cli as cli
from ledit_core.interfaces.tui_app.state import (
    CHOICES,
    DEFAULT_CONFIG,
    DEFAULT_IMAGE_NAME,
    DEFAULT_OUTPUT_DIR,
...
class TuiApp:
    ...
    def menu(self, title: str, items: list[tuple[str, str]], start_index: int = 0) -> int | None:
        ...
        while True:
            ...
            for row, (label, value) in enumerate(items[top : top + visible], start=3):
                ...
                self.stdscr.addnstr(
                    row, max(4, w // 2), self.truncate(right, max(0, w // 2 - 3)), max(0, w // 2 - 3), attr
                )
            self.draw_footer()
            self.stdscr.refresh()
    ...
    def apply_distro_change(self, old_distro: str):
        ...
        if self.config.get("output") in {old_output, str(DEFAULT_OUTPUT_DIR / DEFAULT_IMAGE_NAME)}:
            self.config["output"] = str(DEFAULT_OUTPUT_DIR / provider.default_image_name)
    ...
    def verify_selected_packages_after_distro_change(self):
        ...
        try:
            missing = DistroPackageSearchService().validate_selection(provider.id, branch, arch, packages)
        except Exception as exc:
            self.message(
                "Package validation",
                f"Could not validate selected packages in {provider.label} repos: {exc}",
                error=True,
            )
            return
        checked = [package for package in packages if package not in missing]
        if missing:
            self.message(
                "Package validation",
    ...
    def package_search_screen(self):
        ...
        try:
            results = DistroPackageSearchService().search(
    ...
    def namespace(self, dry_run: bool = False, yes: bool = True) -> SimpleNamespace:
        return SimpleNamespace(
            ...
            legacy_x11_drivers=self.config["legacy_x11_drivers"],
    ...
    def build_screen(self):
        while True:
            try:
                env = env_from_build_args(self.namespace(dry_run=True))
            ...
            items = [
                ...
                (
                    "Profile",
                    f"{self.config['desktop']} / {self.config['display_manager']} / {self.config['bootloader']}",
                ),
    ...
    def usb_screen(self):
        while True:
            ...
            if choice == 1:
                ...
                value = self.prompt(
                    "Manual USB device",
                    self.config["device"] or ("/dev/diskX" if sys.platform == "darwin" else "/dev/sdX"),
                )
                if value:
    ...
    def device_list_screen(self):
        ...
        if not devices:
            self.message(
                "USB devices", "No removable USB-like devices detected. You can still enter a device manually."
            )
            return
        ...
        while True:
            ...
            items = [
                ...
                (
                    ...
                    f"{self.config['desktop']}  dm={self.config['display_manager']}  wms={','.join(self.config['wms']) or 'none'}",
                ),
                (
                    "Network, Wi-Fi, Bluetooth",
                    f"{self.config['network']}  wifi={self.config['wifi']}  bluetooth={self.config['bluetooth']}",
                ),
                (
            ...
            if choice is None or choice == 8:
                ...
                self.edit_fields(
                    ...
                    [
                        ...
                        ("Legacy X11 video drivers", "legacy_x11_drivers", "bool"),
...
def self_test() -> int:
    ...
    ns = SimpleNamespace(
        ...
        root_password=app_config["root_password"],
        ask_password=False,
        timezone=app_config["timezone"],
        locale=app_config["locale"],
        language=app_config["language"],
        console_keymap=app_config["console_keymap"],
        xkb_layout=app_config["xkb_layout"],
        xkb_variant=app_config["xkb_variant"],
        xkb_model=app_config["xkb_model"],
        desktop=app_config["desktop"],
        display_manager=app_config["display_manager"],
        default_session=app_config["default_session"],
        wm=app_config["wms"],
        tiling_wms="",
        browser=app_config["browser"],
        audio=app_config["audio"],
        network=app_config["network"],
        wifi=app_config["wifi"],
        bluetooth=app_config["bluetooth"],
        bootloader=app_config["bootloader"],
        kernel=app_config["kernel"],
        firmware=app_config["firmware"],
        legacy_x11_drivers=app_config["legacy_x11_drivers"],
        boot_timeout=int(app_config["boot_timeout"]),
        systemd_boot_console_mode=app_config["systemd_boot_console_mode"],
        auto_resize=app_config["auto_resize"],
        extra_package=None,
        extra_packages=app_config["extra_packages"],
        dry_run=True,
        yes=True,
    )
    env = env_from_build_args(ns)
    assert env["LEDIT_USB_TILING_WMS"] == "i3 sway"
