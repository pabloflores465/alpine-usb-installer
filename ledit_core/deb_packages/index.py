...
def deb_cache_dir() -> Path:
    explicit = os.environ.get("LEDIT_USB_DEB_CACHE_DIR") or os.environ.get("LINUX_USB_DEB_CACHE_DIR")
    if explicit:
...
def _cache_enabled() -> bool:
    return os.environ.get("LEDIT_USB_DEB_CACHE", "1").lower() not in {"0", "no", "false", "off"}


def _cache_ttl_seconds() -> int:
    raw = os.environ.get("LEDIT_USB_DEB_CACHE_TTL", str(DEFAULT_CACHE_TTL_SECONDS))
    try:
        return int(raw)
    except ValueError:
