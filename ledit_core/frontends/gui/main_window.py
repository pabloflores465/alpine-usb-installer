...
from ledit_core.build_profiles.config_files import ConfigFileError, load_config_file, save_config_file, scrub_config
from ledit_core.frontends.gui.constants import (
    APP_TITLE,
    CONFIG_FILE_FILTER,
    DEFAULT_OUTPUT_DIR,
    DEFAULT_OUTPUT_PATH,
...
from ledit_core.frontends.gui.dialogs import modal
from ledit_core.frontends.gui.icons import make_app_icon, make_button_icon
from ledit_core.frontends.gui.runtime import SCRIPT_DIR
...
from ledit_core.frontends.gui.utils import add_combo_items, combo_value, find_executable, html_soft_break
from ledit_core.frontends.gui.widgets import CollapsibleSection, DeviceDialog, PackageSuggestionList, PasswordLineEdit
from ledit_core.frontends.gui.workers import ApkSearchWorker, BuildWorker, FlashWorker
from ledit_core.image_builds import env_from_build_args
from ledit_core.image_builds.execution import ScriptImageBuildRunner
...
from ledit_core.package_search import DistroPackageSearchService
from ledit_core.usb_devices.detection import device_safety_report
...
class Main(QWidget):
    ...
    def make_config_widgets(self):
        ...
        add_combo_items(self.branch, list(get_distro("alpine").branch_choices))
        self.arch = QComboBox()
    ...
    def update_distro_widgets(self, reset_values: bool = True):
        ...
        current_branch = provider.default_branch if reset_values else self.branch.currentText()
        current_arch = provider.default_arch if reset_values else combo_value(self.arch)
        self.reset_combo_items(self.branch, list(provider.branch_choices), current_branch)
    ...
    def verify_extra_packages_for_current_distro(self):
        ...
        provider = self.current_provider()
        try:
            missing = DistroPackageSearchService().validate_selection(
                provider.id,
                self.branch.currentText().strip() or provider.default_branch,
                combo_value(self.arch) or provider.default_arch,
                packages,
            )
        except Exception as exc:
            modal(self, "error", APP_TITLE, f"Could not validate selected packages in {provider.label} repos: {exc}")
            return
        if missing:
            modal(
                self,
    ...
    def apply_config(self, cfg: dict):
        ...
        self.branch.setCurrentText(str(cfg.get("branch", provider.default_branch)))
    ...
    def write_current_config_silently(self):
        with contextlib.suppress(ConfigFileError):
    ...
    def add_config_sections(self, parent_layout: QVBoxLayout):
        ...
        for key, label, widget in [
            ...
            ("branch", "Branch / release:", self.branch),
    ...
    def search_packages(self):
        ...
        if self.thread_running(self.package_search_worker):
            ...
            return
        provider = self.current_provider()
        branch = self.branch.currentText().strip() or provider.default_branch
    ...
    def collect_build_env(self) -> dict[str, str]:
        ...
        ns = argparse.Namespace(
            ...
            branch=self.branch.currentText().strip() or self.current_provider().default_branch,
            release=None,
            nixos_channel=None,
    ...
    def summary_env_from_config(self, cfg: dict) -> dict[str, str]:
        ...
        return {
            ...
            "branch": str(cfg.get("branch", provider.default_branch)),
    ...
    def build_image(self):
        ...
        self.builder = BuildWorker(env, output_path, ScriptImageBuildRunner(SCRIPT_DIR, DEFAULT_OUTPUT_DIR))
